...
pub fn create_sample_blocks() -> HashMap<Bytes32, Block> {
    let genesis = Block {
        ...
        body: BlockBody { attestations: ssz::PersistentList::default() },
    };
    ...
    HashMap::from([(genesis_hash, genesis), (block_a_hash, block_a), (block_b_hash, block_b)])
}

pub fn create_block(slot: u64, proposer_index: u64, parent_root: Bytes32, state_data: &[u8; 32]) -> Block {
    Block {
        slot: Slot(slot),
...
pub fn create_checkpoint(root: Bytes32, slot: u64) -> Checkpoint {
    Checkpoint { root, slot: Slot(slot) }
}